...
def _unwrap_gateway_payload(body: Any) -> Dict[str, Any]:
    ...
    if "response" in body and isinstance(body["response"], dict):
        resp = body["response"]
        if "prediction" in resp and isinstance(resp["prediction"], dict):
...
def call_model(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ).strip() or os.getenv("CDSW_APIV2_KEY", "")
    ...
    try:
        r = requests.post(url, json=wrapped_payload, headers=headers, timeout=180)
        r.raise_for_status()
        return _unwrap_gateway_payload(r.json())
    except requests.RequestException as e:
        return {
            "error": str(e),
            "hint": "Check CML_MODEL_PREDICT_URL and CDSW_APIV2_KEY/accessKey",
        }
...
def main() -> None:
    ...
    st.caption(
        "Two **HistGradientBoostingRegressor** models: dense monthly series vs sparse intermittent buys."
    )
    ...
    tab_d, tab_s = st.tabs(
        ["Dense forecast (regular buys)", "Sparse forecast (rare buys)"]
    )
